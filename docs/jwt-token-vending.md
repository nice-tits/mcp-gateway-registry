...
The JWT Token Vending Service provides a user-friendly mechanism for generating personal access tokens using the local authentication system. This service bridges the gap between human authentication (web UI sessions) and machine authentication (JWT tokens), enabling users to create tokens with scoped permissions for automation, scripting, and agent access.
...
- **Manual Token Management**: Requiring users to manually generate credentials through external systems creates friction and security risks
...
The JWT Token Vending Service provides an enterprise-ready solution that integrates directly with the local MCP Gateway authentication infrastructure, allowing users to generate scoped JWT tokens through a familiar web interface.
...
graph TB
    ...
    %% Auth flows
    LocalAuth -.->|User session validation<br>Group/scope mapping| AuthServer
...
sequenceDiagram
    ...
    AuthServer->>AuthServer: Generate self-signed JWT with HMAC-SHA256
    ...
    else API Key Auth (Fallback)
        AuthServer->>AuthServer: Standard API key validation
    end
...
1. A **User** authenticates to the Registry web UI using their local session (basic authentication with username/password) which contains their current scopes and permissions from the local user management system.
...
3. The **Registry** validates the user's session, ensures requested scopes are a subset of the user's current permissions (derived from their groups in users.yml), and calls the Auth Server's internal token generation endpoint with the user context and token parameters.
...
8. The **Auth Server** detects self-signed tokens by the issuer claim, validates the HMAC-SHA256 signature using the shared secret, and enforces scope-based access control using the same logic as other authentication methods.
...
try:
    ...
    if unverified_claims.get('iss') == 'mcp-auth-server':
        ...
        # Fall back to API key or basic auth validation
        return validate_other_auth_methods(token)
...
## Local Authentication Benefits

The JWT Token Vending Service works within the local authentication system to provide comprehensive token lifecycle management:

### Self-Contained Token Management

With local authentication, the system provides:

- **No External Dependencies** - All token generation and validation happens locally
- **Immediate Setup** - Works with default admin credentials out of the box
- **Simple Configuration** - No complex OAuth provider setup required
- **Full Control** - Complete control over token lifecycle and user management

### Token Generation Workflow

1. **User Authentication** - Login with local credentials (admin:admin by default)
2. **Token Configuration** - Set expiration, scopes, and description via web UI
3. **Self-Signed Generation** - Tokens signed with local HMAC-SHA256 key
4. **Immediate Usage** - Tokens ready for use with MCP clients immediately

### Enhanced Security Model

The local authentication system provides:

- **Local User Control** - Users managed in `auth_server/users.yml` with bcrypt hashing
- **Configurable Scopes** - Group-based permissions mapped to specific MCP server access
- **Self-Signed Security** - HMAC-SHA256 signed tokens with configurable expiration
- **Centralized Management** - All authentication handled by local auth server

### Usage Pattern

1. **Login to Web Interface** - Use default admin:admin credentials or configured user accounts
2. **Generate JWT Token** - Create tokens via web interface with desired scopes and expiration
3. **Use in Applications** - Tokens work immediately with MCP clients and automation scripts
4. **Local Validation** - All token validation happens locally without external service calls 